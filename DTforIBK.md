...
5G는 오픈 API, 빅데이터, 클라우드 등의 기술이 확산될 수 있도록 지원함으로써 보다 정교화된 금융서비스와 비용 효율성이 극대화된 금융시스템 구축을 지원할 것이다. 5G통신망은 초고속의
초저지연 환경을 제공하기 때문에 폭발적으로 증가하는 고객 데이터를 빠르게 수용하고 의미 있는 정보로 재생산함으로써 빅데이터 기반의 다양한 금융서비스를 창출한다. 


## 오픈API

하나의 웹 사이트에서 자신이 가진 기능을 이용할 수 있도록 공개한 프로그래밍 인터페이스가 오픈 API이다
