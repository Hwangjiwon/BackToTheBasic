...
: 해커톤 진행 멘토님께 조언을 구하고 설계를 줄임
: 해커톤 이후에 LSTM모델을 이용한 주가예측 서비스 추가

5) 기여방안
: 기술적으로는 오픈뱅킹과 딥러닝을 사용해 보았기에 제가 만들고 싶은 신용평가 모델 구축에 기여할 수 있을 것
: 팀워크로는 각자의 개발부분을 설명하는 시간을 가지면서 함께 성장하는 것을 목표로 하였기에 앞으로도 이런 모습 기여가능 
